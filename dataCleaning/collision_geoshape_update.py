...
def add_collision_geoshape_point(index, doc_type):
    ...
    for result in helpers.scan(es,index=index,doc_type=doc_type): 
        if 'collision_GEOJSON_C' in result['_source']:
            idx+=1
            _id = result['_source']['collision_ID']
        ...
        if idx >= 10000:
            upload_to_Elasticsearch.update_ES_records_curl(updates,index=index,doc_type=doc_type,id_field='collision_ID')
            idx=0
            updates=[]
...
def add_zcta_zip_to_collisions():
    ...
    for result in helpers.scan(es,index='saferoad',doc_type='collisions'): 
        ...
        if idx >= 10000:
            upload_to_Elasticsearch.update_ES_records_curl(updates,index='saferoad',doc_type='collisions',id_field='collision_ID')
            idx=0
            updates=[]
