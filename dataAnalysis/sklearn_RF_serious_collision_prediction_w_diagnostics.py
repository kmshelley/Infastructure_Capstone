...
def get_training_test_data(test_date,victim='all',fraction=1.0):
    ...
    trainRDD = sub_acc.union(sub_noacc).map(lambda row: trainingDict(row,victim)).filter(lambda row: row)
    trainRDD.cache()
...
def fit_and_predict_model(train_data,test_data,pred_data,weights={1:0.5,0:0.5},victim='all'):
    ...
    predict_test_probs = rf.predict_proba(X_test)[:,1]
    ...
    return rf,probabilities,feature_idx,diagnostics
...
#fit RF and predict
model,probabilities,all_features,all_diagnostics = fit_and_predict_model(trainRDD,testRDD,predRDD,victim='all')
...
#fit RF and predict
model,probabilities,ped_features,ped_diagnostics = fit_and_predict_model(trainRDD,testRDD,predRDD,victim='pedestrian')
...
model,probabilities,cyc_features,cyc_diagnostics = fit_and_predict_model(trainRDD,testRDD,predRDD,victim='cyclist')
...
model,probabilities,mot_features,mot_diagnostics = fit_and_predict_model(trainRDD,testRDD,predRDD,victim='motorist')
