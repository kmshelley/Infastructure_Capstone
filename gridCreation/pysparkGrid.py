...
##    #input: index, doc_type, and a date-time field
##    #output: returns the earliest ES record based on the datetime field
##    
##    es_url = 'http://%s:%s@%s:9200' % (ES_username,ES_password,ES_url)
##    es = Elasticsearch(es_url)
##
...
def feature_grid(g):
    ...
    g['grid_id'] = g['grid_fullDate'] + '_' + g['grid_zipcode']
...
def add_collisions(g):
    ...
    count = len(list(helpers.scan(es,query=query,index='saferoad',doc_type='collisions'))) #get the collision count for that zip code and hour
    
    g['grid_collision_counter'] = count
    g['grid_isAccident'] = 1 if count > 0 else 0
...
def add_weather(g):
    ...
    if observation:
        ...
        if observation:
            #numerical fields, change 99999 gues to NA
            update['weather_WetBulbFarenheit'] = observation['weather_WetBulbFarenheit']
            update['weather_WindSpeed'] = observation['weather_WindSpeed']
            update['weather_Visibility'] = observation['weather_Visibility']
            update['weather_HourlyPrecip'] = observation['weather_HourlyPrecip']            
        ...
        #otherwise, no weather data, fill gues with NA        
        else:
            #numerical fields, change 99999 gues to NA
...
create_new_grid_timestamps(index='dataframe_test',doc_type='rows')
